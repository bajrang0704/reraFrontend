...
export default function CommonAreasPage() {
    ...
    // Helper to deduplicate facilities based on masterId (System) or Name (Custom)
    // We prioritize the *latest* entry if duplicates exist.
    const deduplicateFacilities = (list: Facility[]): Facility[] => {
        const uniqueMap = new Map<string, Facility>();

        list.forEach(item => {
            // Uniqueness Key: masterId for SYSTEM, name for CUSTOM
            const key = item.scope === 'SYSTEM'
                ? (item.masterId || item.code || item.name)
                : item.name;
        ...
        try {
            ...
            if (commonAreasData && commonAreasData.data) {
                ...
                if (facilities.length > 0) {
                    replace(deduplicateFacilities(facilities));
                }
            } else {
    ...
    const onSubmit: SubmitHandler<CommonAreaFormData> = async (data) => {
        ...
        try {
            ...
            console.log("DEBUG: Facilities in Payload:", payload.facilities.length);
            if (payload.facilities.length > 0) {
                console.log("DEBUG: First Facility Payload:", JSON.stringify(payload.facilities[0]));
            }
            ...
            console.log("DEBUG: Refreshed Common Areas:", refreshedData);
            if (refreshedData?.data?.facilities) {
                console.log("DEBUG: Facilities Count:", refreshedData.data.facilities.length);
                if (refreshedData.data.facilities.length > 0) {
                    console.log("DEBUG: First Facility Response:", JSON.stringify(refreshedData.data.facilities[0]));
                }
                                ...
                                <Table size="small">
                                    ...
                                    <TableBody>
                                        {fields.map((field, index) => {
                                            const isProposed = watchedFacilities?.[index]?.proposed;
                                            return (
