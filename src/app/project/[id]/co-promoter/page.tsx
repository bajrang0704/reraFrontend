...
export default function CoPromoterPage() {
    ...
    const onSubmit = async (data: PromoterFormData) => {
        ...
        try {
            // Build promoter payload (excluding bank details and internal uuid)
            const { bankName, accountNumber, branchName, bankAddress, ifscCode, projectUuid, ...promoterData } = data;
            const payload = {
                ...promoterData,
                officeNumber: promoterData.officeNumber || undefined,
                faxNumber: promoterData.faxNumber || undefined,
                promoterTypeOther: data.promoterType === "OTHERS" ? data.promoterTypeOther : undefined,
            };
            ...
            {selectedProject && (
                <Box component="form" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <Grid container spacing={2} sx={{ mb: 3 }}>
                        ...
                        <Grid size={{ xs: 12, md: 6 }}>
                            <TextField size="small" fullWidth label="Mobile Number *" InputLabelProps={{ shrink: true }} {...register("mobileNumber", { required: "Required", pattern: { value: /^[0-9]{10}$/, message: "Must be 10 digits" } })} helperText={errors.mobileNumber?.message} error={!!errors.mobileNumber} />
                        </Grid>
                        <Grid size={{ xs: 12, md: 6 }}>
